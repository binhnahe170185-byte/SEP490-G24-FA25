...
export default function FeedbackDetail() {
  ...
  const handleReAnalyze = async () => {
    try {
      setUpdating(true);
      await FeedbackApi.reAnalyzeFeedback(id);
      message.success("Re-analysis completed successfully");
      loadFeedback();
    } catch (error) {
      console.error("Failed to re-analyze:", error);
      message.error("Failed to re-analyze");
    } finally {
      setUpdating(false);
    }
  };
      ...
      <Card>
        <Space style={{ marginBottom: "24px", width: "100%", justifyContent: "space-between" }}>
          ...
          <Space>
            ...
            <Button icon={<ReloadOutlined />} onClick={handleReAnalyze} loading={updating}>
              Re-analyze
            </Button>
          </Space>
        </Space>
                    ...
                    Object.keys(feedback.answers).map((questionId) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td style={{ padding: "12px 16px" }}>
                            <Tag color="blue">{answerValue || "N/A"}</Tag>
                          </td>
                        </tr>
                      );
                    })}
