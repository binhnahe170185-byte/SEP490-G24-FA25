...
export default function FilterBar({ year, onYearChange, weekNumber, onWeekChange, onPrev, onNext, weekLabel, disablePrev, disableNext }) {
    ...
    return (
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <Button icon={<LeftOutlined />} onClick={onPrev} disabled={disablePrev} />
            <Select style={{ width: 220 }} value={weekNumber} options={weekOptions} onChange={onWeekChange} />
            <Button icon={<RightOutlined />} onClick={onNext} disabled={disableNext} />
        </div>
    );
}
